  ...
  try {
    ...
    if (mode === "truvern" && !acceptedAcknowledgement) {
      return json(400, {
        ...
        error:
          "Customer acknowledgement acceptance is required for Truvern-managed reviews.",
      });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
        ...
        if (active?.assignmentId && active?.requestId) {
          return {
            ...
            body: {
              ...
              mode,
              redirectUrl: `/review-desk/${active.assignmentId}`,
            },
          };
        }
      ...
      const note =
        mode === "truvern"
          ? "Requested from Review Desk intake."
          : "Started from Review Desk intake.";
      ...
      if (mode === "truvern" && body.acceptedAcknowledgement !== true) {
        return {
          ...
          body: {
            ok: false,
            error:
              "Legal acknowledgement is required before starting a Truvern-managed review.",
          },
        };
      }
      ...
      const requests = await tx.$queryRaw<Array<{ id: number }>>`
        ...
        values (${vendor.organizationId}, ${vendor.id}, ${assessmentId}, ${title}, ${note}, 'REQUESTED'::"ReviewRequestStatus", now())
        returning id
      `;
      ...
      if (mode === "truvern") {
        ...
        for (const opsUser of opsUsers) {
          await createNotification({
            ...
            severity: "INFO",
            title: `New Truvern managed review · ${vendor.name}`,
            message:
      ...
      return {
        ...
        body: {
          ...
          reservation,
          legalAcknowledgement,
          redirectUrl: `/review-desk/${assignment.id}`,
        },
      };
    });
