...
function queueHasOwner(
  ...
  return reviewer !== "Submitted questionnaire" && reviewer !== "Submitted questionnaire";
}
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-200">
            {upper(row?.assignmentType) === "INTAKE" ? `Assessment #${row?.assessmentId ?? assignmentId ?? "—"} · Submitted questionnaire` : `Assignment #${assignmentId ?? "—"} · Truvern Review`}
          </p>
...
export default async function ReviewDeskPage({ searchParams }: Props) {
  ...
  let recentAssignments = await many<AnyRow>`
    select *
    from (
      select
        ra.id,
    ...
    limit 75
  `;
  // local-submitted-assessment-fallback
  if (recentAssignments.length === 0 && process.env.NODE_ENV !== "production") {
    recentAssignments = await many<AnyRow>`
      select
        a.id::int as id,
        a.status::text as status,
        'INTAKE'::text as "assignmentType",
      ...
      left join "AssessmentTemplate" t on t.id = a."templateId"
      where a."isVendorSubmitted" = true
        and a.status in ('SUBMITTED'::"AssessmentStatus", 'REVIEW_READY'::"AssessmentStatus")
      order by coalesce(a."submittedAt", a."reviewReadyAt", a."updatedAt") desc
      limit 75
    `;
  }
...
const submittedQuestionnaireQueue = recentAssignments.filter((row: any) => {
  const source = String(row?.latestResponses?.source ?? "").toLowerCase();
  const assignmentType = String(row?.assignmentType ?? "").toUpperCase();
  const status = String(row?.status ?? "").toUpperCase();

  return (
    source === "vendor_assessment_intake" ||
    assignmentType === "INTAKE" ||
    status === "SUBMITTED" ||
    status === "REVIEW_READY"
  );
});
const creditLedgerRows =
  ...
  searchNeedle === "release-ready"
    ...
    const matchesOwner =
      queueOwner === "all" ||
      (queueOwner === "Submitted questionnaire" && !hasOwner) ||
...
const queueTabs = [
  ...
  const completedEvidenceRequests = vendorId
    ? await countRaw`select count(*)::text as count from "EvidenceRequest" where "vendorId" = ${vendorId} and upper(coalesce(status::text, '')) in ('RECEIVED','COMPLETED','FULFILLED')`
    : 0;
    ...
    ? await prisma.$queryRawUnsafe<
      ...
      >(
        ...
        left join "TruvernCreditLedgerEntry" l
          ...
          and l.status = 'POSTED'::"TruvernCreditEntryStatus"
    ...
    <main className="mx-auto max-w-7xl px-6 py-10">
      ...
      <section className="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6 shadow-[0_0_35px_rgba(34,211,238,0.08)]">
        ...
        <div className="mt-5 grid gap-4">
          {submittedQuestionnaireQueue.slice(0, 6).map((row: any) => (
            <ManagedAssessmentQueueCard key={row.id ?? row.assignmentId ?? row.assessmentId} row={row} />
          ))}
        </div>
      </section>
...
<p className="text-sm uppercase tracking-[0.3em] text-cyan-200">
      ...
      <h1 className="mt-3 text-4xl font-semibold text-white">Governance Ops</h1>
...
<section className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
  <div>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-5">
          ...
          <Link
            href={queueUrl("release_ready", q, queueType, queueOwner, queueSort)}
          ...
          >
            <p className="text-sm text-violet-100">Release readiness</p>
            <p className="mt-3 text-3xl font-semibold text-white">
              {releaseReadyCount}
            </p>
          </Link>
          ...
          <Link
            href={queueUrl("awaiting_confirmation", q, queueType, queueOwner, queueSort)}
          ...
          <Link
            href={queueUrl("confirmed", q, queueType, queueOwner, queueSort)}
          ...
          >
            <p className="text-sm text-emerald-100">Finalized reviews</p>
            <p className="mt-3 text-3xl font-semibold text-white">
              {confirmedCount}
            </p>
          </Link>
  ...
  <div className="mt-3 space-y-2 text-sm">
    ...
    <div className="flex items-center justify-between">
      <span className="text-slate-300">Snapshot missing</span>
    ...
    <div className="flex items-center justify-between">
      <span className="text-slate-300">Unsealed</span>
      <span className="font-semibold text-amber-100">
        {unsealedCount}
          ...
          <div className="rounded-3xl border border-rose-400/15 bg-rose-500/10 p-5">
            <p className="text-sm text-rose-100">Submitted questionnaire</p>
            <p className="mt-3 text-3xl font-semibold text-white">
              {unassignedCount}
            </p>
          </div>
        ...
        >
          ...
          >
            ...
            <option value="Submitted questionnaire">Submitted questionnaires</option>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href={queueUrl(queueView, q, queueType, "Submitted questionnaire", queueSort)}
            className={
              queueOwner === "Submitted questionnaire"
          ...
          >
            Submitted questionnaires · {unassignedCount}
          </Link>
  ...
  {[
    {
      label: "Needs owner",
      href: queueQuickUrl({ view: "active", owner: "Submitted questionnaire", sort: "priority" }),
        ...
        status === "CONFIRMED"
          ...
          : ageHours >= 72
            ...
            : ageHours >= 24
              ? "Aging"
              : "Healthy",
      ownerState: hasOwner ? "Assigned" : "Submitted questionnaire",
      recommendation:
  ...
  <div className="mt-6 grid gap-4">
    {filteredAssignments.length ? (
            filteredAssignments.map((row) => {
              ...
              const reviewer = queueReviewer(row, reviewerNameMap);
              const hasOwner = queueReviewer(row, reviewerNameMap) !== "Submitted questionnaire";
...
/>   
  ...
  <p className="text-lg font-semibold text-white">
    {queueOwner === "Submitted questionnaire"
      ? "No Submitted questionnaire reviews"
      : queueView === "released"
      ...
      {!vendor ? (
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          <p className="text-slate-300">
            Select a review assignment from the queue, or open Governance Ops from a
            vendor profile.
          </p>
        </div>
      ) : (
