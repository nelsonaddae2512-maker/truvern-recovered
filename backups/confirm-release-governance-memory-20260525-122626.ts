...
function safeSnapshotObject(value: unknown): Record<string, any> {
...
try {
    ...
    const nextResponses = {
      ...
      governanceReleaseSnapshot: {
        ...
        findingsSnapshot:
          existing.truvernReviewerIntelligence?.findings ?? [],

        autoRiskScoreSnapshot:
          existing.truvernReviewerIntelligence?.autoRiskScore ?? null,

        executiveSummarySnapshot:
          existing.truvernReviewerIntelligence?.executiveSummary ?? null,

        finalRecommendationSnapshot:
          existing.truvernReviewerIntelligence?.finalRecommendation ?? null,

        governanceTimelineSnapshot:
          existing.truvernReviewerIntelligence?.timeline ?? [],

        followUpsSnapshot:
          existing.truvernReviewerIntelligence?.followUps ?? [],

        remediationStatusSnapshot:
          existing.truvernRemediation?.remediationStatus ?? null,

        remediationHistorySnapshot:
          existing.truvernRemediation?.history ?? [],

        remediationItemsSnapshot:
          existing.truvernRemediation?.remediationItems ?? [],

        attestationRequestsSnapshot:
          existing.truvernRemediation?.attestationRequests ?? [],

        reviewerConditionsSnapshot:
          existing.truvernRemediation?.reviewerConditions ?? [],
        ...
        governanceDecisionSnapshot:
          existing.truvernRemediation?.reviewerDecision ?? null,
