...
import ReviewDeskIntakeActions from "@/components/review-desk/review-desk-intake-actions.client";
...
function queueHasOwner(
  ...
  return queueReviewer(row, reviewerNameMap) !== "Unassigned";
}
...
function getManagedAssessmentMetrics(row: any) {
  const responsePayload = safeObject(row?.response?.responses);
...
function ManagedAssessmentQueueCard({ row }: { row: any }) {
  ...
  const reviewHref = `/review-desk/${row?.id}`;
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-cyan-200">
            Assignment #{row?.id ?? "—"} · Managed vendor assessment
          </p>
...
export default async function ReviewDeskPage({ searchParams }: Props) {
  ...
  // Inline Review Desk workspace stays on this page.
  ...
  const recentAssignments = await many<AnyRow>`
    ...
    from (
      ...
      select
        ...
        coalesce(a.title, t.name, 'Vendor assessment') as "assessmentTitle",
        null::int as "latestResponseId",
        null::int as "manifestId",
        jsonb_build_object(
          'source', 'vendor_assessment_intake',
          'assessmentId', a.id,
          'assessmentTitle', coalesce(a.title, t.name, 'Vendor assessment'),
        ...
        and not exists (
          ...
          where rr2."assessmentId" = a.id
            and ra2.status in (
              ...
              'IN_PROGRESS'::"ReviewAssignmentStatus",
              'SUBMITTED'::"ReviewAssignmentStatus"
            )
        )
    ) queue
...
const creditLedgerRows =
  ...
  searchNeedle === "release-ready"
    ...
    const matchesOwner =
      queueOwner === "all" ||
      (queueOwner === "unassigned" && !hasOwner) ||
...
return (
    <main className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm text-slate-300">
              Submitted vendor assessments requiring Truvern reviewer intelligence: due date, completion, requester, vendor, SLA aging, auto-risk score, reopen requests, findings, remediation, and immutable release readiness.
            </p>
          </div>
      ...
      {/* Managed vendor assessment queue cards */}
      <section className="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6 shadow-[0_0_35px_rgba(34,211,238,0.08)]">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-4xl text-sm text-slate-300">
              Submitted vendor assessments with due date, completion, requester, vendor, SLA aging, auto-risk score, reopen requests, findings, remediation, and release readiness.
            </p>
          </div>
        </div>
        ...
        <div className="mt-5 grid gap-4">
          {filteredAssignments.slice(0, 6).map((row: any) => (
            <ManagedAssessmentQueueCard key={row.id} row={row} />
          ))}
        </div>
      </section>
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.3em] text-cyan-200">
              Truvern Managed Assessments
            </div>

            <h2 className="mt-3 text-2xl font-black tracking-tight text-white">
              Turn vendor intake into managed governance delivery.
            </h2>
...
<p className="text-sm uppercase tracking-[0.3em] text-cyan-200">
      ...
      <h1 className="mt-3 text-4xl font-semibold text-white">Review Desk</h1>
...
<section className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
      ...
      <span className="font-semibold text-amber-100">
        {unsealedCount}
          ...
          <div className="rounded-3xl border border-rose-400/15 bg-rose-500/10 p-5">
            <p className="text-sm text-rose-100">Unassigned</p>
            <p className="mt-3 text-3xl font-semibold text-white">
              {unassignedCount}
            </p>
          </div>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href={queueUrl(queueView, q, queueType, "unassigned", queueSort)}
            className={
  ...
  {[
    {
      label: "Needs owner",
      href: queueQuickUrl({ view: "active", owner: "unassigned", sort: "priority" }),
        ...
        status === "CONFIRMED"
          ...
          : ageHours >= 72
            ...
            : ageHours >= 24
              ? "Aging"
              : "Healthy",
      ownerState: hasOwner ? "Assigned" : "Unassigned",
      recommendation:
  ...
  <div className="mt-6 grid gap-4">
    {filteredAssignments.length ? (
            filteredAssignments.map((row) => {
              ...
              const hasOwner = queueReviewer(row, reviewerNameMap) !== "Unassigned";
      ...
      {!vendor ? (
        <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          <p className="text-slate-300">
            Select a review assignment from the queue, or open Review Desk from a
            vendor profile.
          </p>
        </div>
      ) : (
...
generatedDraft: {
              ...
              auditEvents={[
  {
    ...
    detail:
      "Review assignment was created from Review Desk intake.",
  },
