...
export async function POST(
  ...
  try {
    ...
    const result =
      await rotateDeploymentLicenseKey({
        id: licenseId,
        actorUserId,
        reason,
      });
    ...
    /*
     * The replacement credential is returned
     * exactly once by this operation.
     *
     * Neither GET endpoints nor other lifecycle
     * mutation endpoints may return licenseKey.
     */
    return json(200, {
      ok: true,
      license:
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Failed to rotate deployment license key.";
