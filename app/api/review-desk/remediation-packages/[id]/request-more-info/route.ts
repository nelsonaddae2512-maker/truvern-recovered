...
export async function POST(request: Request, props: Props) {
  try {
    ...
    const result = await emitWorkflowEvent({
      event: WorkflowEvent.MoreInformationRequested,
      packageId,
      actor: body?.actor || "TRUVERN_REVIEWER",
      summary: body?.summary || "Truvern requested more information for this remediation package.",
      payload: {
        comment: body?.comment || body?.rationale || "",
    ...
    return NextResponse.json(
      { ok: false, error: String(error?.message || "Failed to request more information.") },
      { status: 500 },
    );
  }
}
