...
export default async function VendorDetailPage({
  ...
  searchParams?: Promise<{ managedReview?: string; assignmentId?: string }>;
  ...
  const governanceMetrics = governanceMetricRows[0] || {
    evidenceCount: vendor._count.evidence,
    evidenceRequestCount: vendor._count.evidenceRequests,
    activeReviewCount: activeAssessments,
    issueCount: vendor._count.issues,
  };
  const score =
  ...
  const managedReviewCreated = resolvedSearchParams?.managedReview === "created";
  const managedReviewAssignmentId =
    resolvedSearchParams?.assignmentId || null;
  const currentPlanTier = await getCurrentOrgPlanTier();
  const canAccessGovernanceReleaseArchive =
    currentPlanTier === "PRO" ||
    currentPlanTier === "ENTERPRISE";
  const canAccessFullGovernanceArtifacts =
    currentPlanTier === "ENTERPRISE";
...
return (
    <main className="mx-auto max-w-7xl px-6 py-10">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-black tracking-tight text-white">
              Manage this vendor reviews, releases, evidence, and governance history.
            </h2>

            <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">
              Review all assessment activity, launch Truvern Reviews, inspect immutable
      ...
      >
        <span aria-hidden="true">&larr;</span> Back to vendors
      </Link>

      


      <VendorReviewPath vendorId={vendor.id} />

      <div className="sticky top-20 z-30 mt-6 overflow-x-auto rounded-2xl border border-cyan-400/20 bg-[#031525]/90 px-4 py-3 backdrop-blur-xl">
        <div className="flex min-w-max items-center gap-3">
          {[
            ...
            <a
              key={href}
              href={`#${href}`}
              className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-semibold uppercase tracking-[0.22em] text-slate-200 transition hover:border-cyan-300/40 hover:bg-cyan-400/10 hover:text-cyan-100"
            >
              {label}
            </a>
          ))}
...
<div id="vendor-profile" className="scroll-mt-40"></div>
<div id="overview" className="mt-6 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
      ...
      {managedReviewCreated ? (
        <div className="mt-6 rounded-3xl border border-emerald-300/30 bg-emerald-400/10 p-5 text-emerald-50">
          <p className="text-xs uppercase tracking-[0.28em] text-emerald-200">
            Truvern Review started
          </p>
          <p className="mt-2 text-lg font-semibold text-white">
            Truvern Ops has received this vendor review.
          </p>
          <p className="mt-2 text-sm leading-6 text-emerald-50/80">
            {managedReviewAssignmentId
              ? `Assignment #${managedReviewAssignmentId} has been added to this vendor governance history.`
              : "The assignment has been added to this vendor governance history."}
          </p>
        </div>
      ) : null}

      <div className="mt-8 grid gap-5 md:grid-cols-4">
        <Metric label="Evidence" value={governanceMetrics.evidenceCount} />
        <Metric label="Requests" value={governanceMetrics.evidenceRequestCount} />
        <Metric label="Active reviews" value={governanceMetrics.activeReviewCount} />
        <Metric label="Issues" value={governanceMetrics.issueCount} />
      </div>

      <section id="governance-context" className="mt-8 grid gap-5 rounded-[2rem] border border-cyan-400/15 bg-cyan-500/[0.04] p-4 lg:grid-cols-[0.9fr_1.1fr]">
      ...
      {/* GOVERNANCE_RELEASE_ARCHIVE_TIER_GATE */}
      {!canAccessGovernanceReleaseArchive ? (
        <section className="rounded-3xl border border-amber-300/20 bg-amber-400/10 p-6">
          ...
          <p className="mt-3 max-w-4xl text-sm leading-7 text-amber-50/90">
            Upgrade to Pro or request Truvern Review to access release packets, PDF exports, attestations, and immutable audit archives.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <Link href="/plans" className="rounded-2xl border border-amber-300/30 bg-amber-400/15 px-4 py-3 text-sm font-semibold text-white hover:bg-amber-400/20">
              Upgrade to Pro
            </Link>
            <Link href="/vendors" className="rounded-2xl border border-cyan-300/20 bg-cyan-400/10 px-4 py-3 text-sm font-semibold text-cyan-50 hover:bg-cyan-400/15">
              Request Truvern Review
            </Link>
          </div>
        </section>
      ) : (
        <details className="rounded-3xl border border-emerald-300/20 bg-emerald-400/5 p-6">
          <summary className="cursor-pointer text-sm font-semibold uppercase tracking-[0.24em] text-emerald-100">
            Compliance Artifacts
          </summary>
          <div className="mt-5">
      <section id="release-archive" className="mt-8 rounded-3xl border border-emerald-400/25 bg-emerald-500/[0.08] p-6 shadow-2xl shadow-emerald-950/20">
      ...
      <section id="reviews" className="mt-8 rounded-3xl border border-indigo-400/25 bg-indigo-500/[0.08] p-6 shadow-2xl shadow-indigo-950/20">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-cyan-200">
              Assessment registry
            ...
            <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-300">
              All Truvern Reviews, release states, immutable packets, reviewer ownership,
            ...
            <table className="w-full min-w-[1100px] border-collapse text-left text-sm">
              ...
              <tbody>
                {assessmentRegistryRows
                  .filter((row) => {
                    const state = String(
                      row.releaseState || row.status || "UNDER REVIEW"
                    ).toUpperCase();
...
return (
      ...
      <div id="portal-access" className="mt-8 rounded-3xl border border-violet-400/20 bg-violet-500/[0.06] p-4"><VendorPortalAccessPanel vendorId={vendor.id} /></div>
<section
        id="start-assessment"
        className="mt-8 rounded-3xl border border-sky-300/25 bg-sky-400/[0.08] p-6 shadow-2xl shadow-sky-950/20"
      >
