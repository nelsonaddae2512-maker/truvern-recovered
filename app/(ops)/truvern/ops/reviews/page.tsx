...
import {
  readOpsRecentReviews,
  readOpsReleaseReadyReviews,
  readOpsReviewMetrics,
  readOpsUnclaimedReviews,
} from "@/lib/repositories/truvern-ops-reviews-repository";
...
export default async function TruvernOpsReviewsPage() {
  ...
  const metricRows: MetricRow[] = await readOpsReviewMetrics();
  ...
  const unclaimedReviews: QueueRow[] = await readOpsUnclaimedReviews();

  const releaseReadyReviews: QueueRow[] = await readOpsReleaseReadyReviews();

  const recentReviews: QueueRow[] = await readOpsRecentReviews();
