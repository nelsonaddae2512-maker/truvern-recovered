...
export default async function ReviewEngagementPage({ params }: Props) {
  ...
  const openRemediationRequests = pendingEvidenceRequests;
  const releaseBlocked = openRemediationRequests > 0;

  const evidenceRemediationRequests = await prisma.$queryRaw<Array<{
    id: number;
    title: string | null;
    label: string | null;
    kind: string | null;
    ...
    select
      ...
      ev."fileUrl" as "evidenceFileUrl",
      ev.url as "evidenceUrl",
      ev.notes as "evidenceNotes"
    from "EvidenceRequest" er
    left join "Evidence" ev
      on ev.id = coalesce(er."fulfilledEvidenceId", er."evidenceId")
    where er."vendorId" = ${vendorId}
      and upper(coalesce(er.status::text, '')) in ('REQUESTED','SUBMITTED','OPEN','APPROVED','REJECTED')
    order by er."createdAt" desc, er.id desc
  `;


  const remediationPackages = await prisma.$queryRaw<Array<{
    id: number;
  ...
  const submittedAssessmentAnswers =
    submittedAssessment?.answers?.map((answer) => ({
      ...
      assessmentStatus: String(submittedAssessment.status ?? ""),
      score: submittedAssessment.score ?? null,
      questionId: answer.questionId,
      prompt:
        answer.question?.text ||
        answer.question?.description ||
        `Question #${answer.questionId}`,
      questionType: String(answer.question?.type ?? "TEXT"),
      value:
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-8 space-y-6">
          ...
          <ReviewAssignmentWorkspace
            ...
            vendorAnswers={submittedAssessmentAnswers.length > 0 ? submittedAssessmentAnswers : frameworkVendorAnswers}
            analysts={reviewAnalysts.map((analyst) => ({
            ...
            remediationPackages={remediationPackages.map((pkg) => ({
              id: pkg.id,
              evidenceRequestId: pkg.evidenceRequestId,
              title: safeStr(pkg.title),
              status: upper(pkg.status) || "REQUESTED",
              severity: safeStr(pkg.severity),
              dueAt: iso(pkg.dueAt),
              createdAt: iso(pkg.createdAt),
              updatedAt: iso(pkg.updatedAt),
              payload:
                pkg.payload && typeof pkg.payload === "object"
                  ? pkg.payload
                  : {},
            }))}            remediationRequests={evidenceRemediationRequests.map((request) => ({
              id: request.id,
              title: safeStr(request.title) || safeStr(request.label) || "Evidence request",
              kind: safeStr(request.kind) || "Evidence",
              status: upper(request.status) || "REQUESTED",
              blockerStatus: upper(request.status) || "REQUESTED",
              dueAt: iso(request.dueAt),
              createdAt: iso(request.createdAt),
              updatedAt: iso(request.updatedAt),
              requiredEvidence: [
                safeStr(request.description) ||
                  safeStr(request.notes) ||
                  "Evidence requested from vendor.",
              ],
              requiredAttestation: [],
              releaseImpact: "Release blocked until this evidence request is resolved.",
              evidenceStatus: upper(request.status) || "REQUESTED",
              evidenceId: request.evidenceId ?? request.fulfilledEvidenceId ?? null,
              evidenceTitle: safeStr(request.evidenceTitle),
              evidenceFileUrl: safeStr(request.evidenceFileUrl) || safeStr(request.evidenceUrl),
              evidenceNotes: safeStr(request.evidenceNotes),
            }))}
            latestOutcome={{
              ...
              executiveSummary:
                safeStr(latestOutcomeResponses?.executiveSummary) ||
                safeStr(
                  latestOutcomeResponses?.structuredAssessment?.executiveSummary,
                ) ||
                "",

              finalAssessment:
                safeStr(latestOutcomeResponses?.finalAssessment) ||
                safeStr(
                  latestOutcomeResponses?.structuredAssessment?.finalAssessment,
                ) ||
                "",
 ...
 updatedAt: iso(latestOutcome?.updatedAt),
              generatedDraft: {
                ...
                structuredAssessment: {
                  ...
                  generatedFindingsAt: safeStr(
                    latestOutcomeResponses?.generatedFindingsAt,
                  ),
                },
              },
            }}
