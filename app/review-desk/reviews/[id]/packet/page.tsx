...
function releaseSnapshot(payload: any) {
  return payload?.governanceReleaseSnapshot ?? payload?.releaseSnapshot ?? {};
}
...
export default async function GovernancePacketPage({ params }: Props) {
  ...
  const narrativeSnapshot = releaseSnapshot(responses);

  const intelligence = reviewerIntelligence(responses);

  const executiveSummary = firstNarrativeValue(
    governanceReleasePackage?.executiveSummary,
    governanceReleasePackage?.canonicalGovernanceArtifact?.executiveSummary,
    canonicalGovernanceArtifact?.executiveSummary,
    narrativeSnapshot.executiveSummary,
    narrativeSnapshot.governanceSummary,
    responses.executiveSummary,
    intelligence.executiveSummary,
  );

  const finalAssessment = firstNarrativeValue(
    governanceReleasePackage?.finalAssessment,
    governanceReleasePackage?.finalRecommendation,
    governanceReleasePackage?.canonicalGovernanceArtifact?.finalAssessment,
    governanceReleasePackage?.canonicalGovernanceArtifact?.finalRecommendation,
    canonicalGovernanceArtifact?.finalAssessment,
    canonicalGovernanceArtifact?.finalRecommendation,
    narrativeSnapshot.finalAssessment,
    narrativeSnapshot.finalRecommendation,
  ...
  const decision =
    safeStr(governanceReleasePackage?.decision) ||
    safeStr(canonicalGovernanceArtifact?.decision) ||
    safeStr(responses.decision) ||
    "Not recorded";
  const riskLevel =
    safeStr(governanceReleasePackage?.riskLevel) ||
    safeStr(canonicalGovernanceArtifact?.riskLevel) ||
    safeStr(responses.riskLevel) ||
    "Not recorded";
  const releaseState =
    safeStr(governanceReleasePackage?.metadata?.releaseState) ||
...
const customerConfirmedAt =
  ...
  snapshot?.customerAcknowledgement?.acceptedAt ||
  responses?.customerAcknowledgement?.acceptedAt;

const governanceSealedAt =
