import {
  ...
  readGovernanceReleaseGateAssignment,
...
export async function runGovernanceReleaseGateForAssignment(
  reviewAssignmentId: number,
) {
  const assignments: any[] =
    await readGovernanceReleaseGateAssignment(reviewAssignmentId);
  ...
  for (const assignment of assignments) {
    ...
    const totalPackages =
      Number(rows?.[0]?.totalPackages ?? 0);

    const readyPackages =
      Number(rows?.[0]?.readyPackages ?? 0);

    const blockedPackages =
      Number(rows?.[0]?.blockedPackages ?? 0);
