...
export async function runReleaseReadinessForPackage(
  packageId: number,
) {
  const packages: any[] =
    await readReleaseReadinessPackage(packageId);
  ...
  for (const pkg of packages) {
    ...
    const taskRows: any[] =
      await readReleaseReadinessTaskCounts(pkg.packageId);
    ...
    const readinessState =
      isReady ? "READY_FOR_RELEASE" : "RELEASE_BLOCKED";
